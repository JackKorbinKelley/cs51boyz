...
module ListQueue(C : COMPARABLE) : (PRIOQUEUE with type elt = C.t) =
  struct
    ...
    let empty = failwith "ListQueue empty not implemented"
			 
    let is_empty (t : queue) = failwith "ListQueue is_empty not implemented"
     ...
     *)
    let add (e : elt) (q : queue) = failwith "ListQueue add not implemented"
						 
    let take (q : queue) = failwith "ListQueue take not implemented"
				    
    let run_tests () = failwith "ListQueue run_tests not implemented"
  end
...
module BinaryHeap(C : COMPARABLE) : (PRIOQUEUE with type elt = C.t) =
  struct
    ...
    let take (q : queue) : elt * queue =
      ...
      | TwoBranch (Odd, e, t1, t2) -> failwith "BinaryHeap take incomplete"
...
let minutes_spent () = failwith "please report minutes spent" ;;
